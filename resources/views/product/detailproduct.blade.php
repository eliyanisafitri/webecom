...
<head>
    ...
    <!-- favicon -->
    <link rel="shortcut icon" type="image/png" href="{{ url('main/assets/img/favicon.png') }}">
    <!-- google font -->
    ...
    <!-- bootstrap -->
    <link rel="stylesheet" href="{{ url('main/assets/bootstrap/css/bootstrap.min.css') }}">
    <!-- owl carousel -->
...
<body>
    ...
    <div class="single-product mt-150 mb-150">
        <div class="container">
            <div class="row">
                <?php
                // $value = DB::table('tbstok')->where('id', $id)->first();
                $value = DB::table('tbstok')->where('id', $id)->first();
                        ...
                        <div class="carousel-inner">
                            @if (is_array($fotos) && !empty($fotos))
                                @foreach ($fotos as $index => $foto)
                                    <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                        <img src="{{ asset($foto) }}" alt="Product Image {{ $index + 1 }}"
                                            class="d-block w-100"
                            ...
                            @else
                                <div class="carousel-item active">
                                    <img src="{{ asset('uploads/stok/' . $value->foto) }}" alt="Product Image"
                                        class="d-block w-100" style="width: 400px; height: 400px; object-fit: cover;">
                                </div>
                            @endif
                        </div>
                ...
                <div class="col-md-7">
                    <div class="single-product-content">
                        <h3>{{ $value->nama }}</h3>
                        <p class="single-product-pricing">{{ $value->hargajual }}</p>
                        <p>{{ $value->desc }}</p>
                        <div class="single-product-form">
                            <form action="{{ url('addcart') }}" method="POST">
                                @csrf
                                <div class="alert alert-success" role="alert">
									Total Stok : {{ $value->saldoakhir }}
                                </div>
                                <input type="number" name="qty" placeholder="1" min="1"
                                    max="{{ $value->saldoakhir }}" value="1"
                                    style="display: block; margin-bottom: 10px;">
                                <input type="hidden" name=idstok value={{ $value->id }}>
    ...
    <!-- magnific popup -->
    <script src="{{ url('main/assets/js/jquery.magnific-popup.min.js') }}"></script>
    <!-- mean menu -->
